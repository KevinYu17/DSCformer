...
class DSConv_pro(nn.Module):
    def __init__(
        ...
        if self.offset_mode == '9':
            self.offset_conv = nn.Conv2d(in_channels, 2 * kernel_size, 9, padding=4)
        if self.offset_mode == "pyramid":
            # pyramid kernel
            self.offset_conv3 = nn.Conv2d(in_channels, 6, 3, padding=1)
            self.offset_conv5 = nn.Conv2d(in_channels, 4, 5, padding=2)
            self.offset_conv7 = nn.Conv2d(in_channels, 4, 7, padding=3)
            self.offset_conv9 = nn.Conv2d(in_channels, 4, 9, padding=4)
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.offset_mode == '3' or self.offset_mode == '9':
            offset = self.offset_conv(input)
        else:
            # pyramid kernel
            offset3 = self.offset_conv3(input)
            offset5 = self.offset_conv5(input)
            offset7 = self.offset_conv7(input)
            offset9 = self.offset_conv9(input)
            offset_y = torch.cat(
                (offset9[:, 0].unsqueeze(1), offset7[:, 0].unsqueeze(1), offset5[:, 0].unsqueeze(1), offset3[:, :3],
                 offset5[:, 1].unsqueeze(1), offset7[:, 1].unsqueeze(1), offset9[:, 1].unsqueeze(1)),
                dim=1)
            offset_x = torch.cat(
                (offset9[:, 2].unsqueeze(1), offset7[:, 2].unsqueeze(1), offset5[:, 2].unsqueeze(1), offset3[:, 3:],
                 offset5[:, 3].unsqueeze(1), offset7[:, 3].unsqueeze(1), offset9[:, 3].unsqueeze(1)),
                dim=1)
...
if __name__ == "__main__":
    aa = torch.arange(5, device="cuda", dtype=torch.float)
    aa = einops.repeat(aa, "w ->b k w h", b=1, k=4, h=5)
    conv = DSConv_pro(4, 4, 3, 1.0, 0)
    conv = conv.to(dtype=torch.float)
    conv = conv.cuda()
    out = conv(aa)
    print(out)
